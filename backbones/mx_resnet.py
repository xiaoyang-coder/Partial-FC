...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck, **kwargs):
    ...
    if version_input == 0:
        ...
        body = Conv(data=data, num_filter=filter_list[0], kernel=(3, 3), stride=(1, 1), pad=(1, 1),
            no_bias=True, name="conv0", workspace=workspace)
        body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, namae='bn0')
        body = Act(data=body, act_type=act_type, name='relu0')
    else:
        # data = mx.sym.identity(data=data, name='id')
        # data = data-127.5
    ...
    # gpunum = 8
    # body = mx.sym.contrib.SyncBatchNorm(data=body, ndev=gpunum, key='key_bn1', fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn1')
    ...
    # fc1 = mx.sym.contrib.SyncBatchNorm(data=fc1, ndev=gpunum, key='key_fc1', fix_gamma=True, eps=2e-5, momentum=bn_mom, name='fc1')
    return fc1
